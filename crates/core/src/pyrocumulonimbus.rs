    ...
    pub fn check_formation(
        ...
        if PyroCb::can_form(fire_intensity, ambient_temp, humidity, wind_speed) {
            ...
            let too_close = self
                .clouds
                .iter()
                .any(|cloud| cloud.active && (cloud.position - position).magnitude() < 5000.0);

            if !too_close {
                let cloud = PyroCb::new(position, fire_intensity, ambient_temp);
                println!(
                    "🌩️  PYROCUMULONIMBUS FORMATION at ({:.0}, {:.0})",
                    position.x, position.y
                );
                println!(
                    "   Cloud height: {:.0}m, Updraft: {:.1}m/s, Diameter: {:.0}m",
    ...
    pub fn update(
        ...
        for cloud in &mut self.clouds {
            ...
            if let Some(strike) = cloud.generate_lightning(rng) {
                println!(
                    "⚡ LIGHTNING STRIKE at ({:.0}, {:.0}), energy: {:.0} kJ",
            ...
            if let Some(downdraft) = cloud.generate_downdraft(rng) {
                println!(
                    "💨 DOWNDRAFT from pyroCb, wind speed: {:.0} m/s, radius: {:.0}m",
