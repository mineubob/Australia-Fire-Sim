...
impl FireSimulationUltra {
    ...
    pub fn update(&mut self, dt: f32) {
        ...
        let burning_positions: Vec<Vec3> = self.burning_elements.iter()
        ...
        for element_id in elements_to_process {
            // 4a. Apply grid conditions to element (needs both borrows separate)
            {
                let grid_data = self.grid.interpolate_at_position(
                    self.get_element(element_id).map(|e| e.position).unwrap_or(Vec3::zeros())
                );
                ...
                if let Some(element) = self.get_element_mut(element_id) {
                    ...
                    if grid_data.humidity > element.moisture_fraction {
                        ...
                        let moisture_increase = (grid_data.humidity - element.moisture_fraction) * moisture_uptake_rate;
                        element.moisture_fraction = (element.moisture_fraction + moisture_increase).min(element.fuel.base_moisture * 1.5);
                    }
                    ...
                    if grid_data.suppression_agent > 0.0 {
                        ...
                        element.temperature = (element.temperature - temp_drop).max(grid_data.temperature);
                    }
                }
            }
            ...
            if let Some((pos, temp, fuel_remaining, surface_area, heat_content)) = element_data {
                ...
                if let Some(cell) = self.grid.cell_at_position_mut(pos) {
                    ...
                    // Combustion products
                    let products = crate::physics::combustion_physics::calculate_combustion_products(
                        fuel_consumed,
                        cell,
                        heat_content,
                    );
        ...
        self.suppression_droplets.par_iter_mut().for_each(|droplet| {
        ...
        for droplet in &self.suppression_droplets {
            if droplet.active {
                crate::physics::suppression_physics::apply_suppression_to_grid(droplet, &mut self.grid);
            }
        }
...
mod tests {
    ...
    fn test_simulation_update() {
        ...
        let cell = sim.get_cell_at_position(Vec3::new(50.0, 50.0, 1.0)).unwrap();
