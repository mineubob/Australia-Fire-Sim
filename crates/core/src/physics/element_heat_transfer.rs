...
#[allow(dead_code)] // Kept for tests and fallback; production code uses calculate_heat_transfer_raw_t4
...
/// OPTIMIZED: Heat transfer calculation with pre-computed T^4 values
/// This version accepts pre-computed temperature^4 values to avoid expensive powi(4) in hot loop
/// Physics are IDENTICAL to calculate_heat_transfer_raw - only the T^4 computation is hoisted out
#[inline(always)]
#[expect(
    clippy::too_many_arguments,
    reason = "Performance-critical hot path - pre-computed values eliminate redundant powi(4)"
)]
#[allow(dead_code)] // Available for future optimization when T^4 pre-computation bug is fixed
pub(crate) fn calculate_heat_transfer_raw_t4(
    source_pos: Vec3,
    source_temp: Celsius,
    source_temp_k_pow4: f64,  // Pre-computed source temperature^4 in Kelvin
    source_fuel_remaining: f32,
    source_flame_area_coeff: f32,
    target_pos: Vec3,
    target_temp: Celsius,
    target_temp_k_pow4: f64,  // Pre-computed target temperature^4 in Kelvin
    target_surface_area_vol: f32,
    target_absorption_base: f32,
    wind: Vec3,
    dt: f32,
    ...
    let (_diff, distance) = if tilt_fraction > 0.001 && wind_speed_ms > 0.5 {
        let wind_dir = Vec3::new(wind.x / wind_speed_ms, wind.y / wind_speed_ms, 0.0);
        let horizontal_dist = original_distance_sq.sqrt();
        let flame_tilt_distance = horizontal_dist * tilt_fraction;
    ...
    // === RADIATION CALCULATION (Stefan-Boltzmann) ===
    // Use pre-computed T^4 values - this is the key optimization
    let radiant_power_f64 =
        STEFAN_BOLTZMANN * EMISSIVITY * (source_temp_k_pow4 - target_temp_k_pow4);
    ...
    // View factor and radiation calculation (identical to original)
    let flame_area = source_fuel_remaining * source_flame_area_coeff;
    let view_factor = (flame_area / (std::f32::consts::PI * distance * distance)).min(MAX_VIEW_FACTOR);
    
    let absorption_efficiency = calculate_absorption_efficiency(
        target_absorption_base,
        target_surface_area_vol,
    );

    let radiation = radiant_power * view_factor * absorption_efficiency;
    ...
    let convection = if vertical_diff > 0.0 {
        let temp_source_k = source_temp.to_kelvin();
        let temp_target_k = target_temp.to_kelvin();
        let temp_diff = *temp_source_k - *temp_target_k;
        if temp_diff > 0.0 {
            let convection_coeff = 25.0;
            let distance_attenuation = 1.0 / (1.0 + distance * distance);
            (convection_coeff * temp_diff as f32 * absorption_efficiency * distance_attenuation * 0.001).max(0.0)
        } else {
            0.0
        }
    } else {
    ...
    let wind_factor = if alignment > 0.3 {
        1.0 + alignment * 25.0
    } else if alignment < -0.3 {
        (1.0 + alignment * 0.95).max(0.05)
    } else {
        1.0
    };

    // === VERTICAL/SLOPE SPREAD MULTIPLIER ===
    let vertical_factor = if vertical_diff > 0.0 {
        let slope_boost = (vertical_diff / (original_distance_sq.sqrt() + 0.1)).min(1.0);
        1.0 + slope_boost * 1.5
    } else {
        1.0
    };

    let directional_factor = vertical_factor.max(1.0);
