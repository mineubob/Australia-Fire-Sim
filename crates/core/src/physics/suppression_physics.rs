...
impl SuppressionDroplet {
    ...
    pub fn update(&mut self, wind: Vec3, ambient_temp: f32, dt: f32) {
        ...
        let drag_force = 0.5 * AIR_DENSITY * drag_coeff * cross_section 
...
impl AircraftDrop {
    ...
    pub fn generate_droplets(&self, dt: f32) -> Vec<SuppressionDroplet> {
        ...
        for i in 0..num_droplets {
            ...
            let vel = self.velocity + Vec3::new(
                0.0,
                0.0,
                -5.0,  // Initial downward velocity
            );
    ...
    pub fn generate_droplets(&self, dt: f32) -> Vec<SuppressionDroplet> {
        ...
        for i in 0..num_droplets {
            ...
            let lateral = Vec3::new(
                cone_angle * angle.cos(),
                cone_angle * angle.sin(),
                0.0,
            );
...
pub fn apply_suppression_to_grid(
    droplet: &SuppressionDroplet,
    ...
    if let Some(cell) = grid.cell_at_position_mut(droplet.position) {
        ...
        if matches!(droplet.agent_type, SuppressionAgent::Water | SuppressionAgent::Foam) {
...
mod tests {
    ...
    fn test_grid_application() {
        ...
        let agent = grid.cell_at_position(droplet.position).unwrap().suppression_agent;
        assert!(agent > 0.0);
    }
    
    #[test]
    fn test_cooling_capacity() {
        let water = SuppressionDroplet::new(
            Vec3::zeros(),
            Vec3::zeros(),
            1.0,
            SuppressionAgent::Water,
        ...
        let foam = SuppressionDroplet::new(
            Vec3::zeros(),
            Vec3::zeros(),
            1.0,
            SuppressionAgent::Foam,
    ...
    fn test_suppression_effectiveness() {
        ...
        let eff_retardant = calculate_suppression_effectiveness(0.5, SuppressionAgent::LongTermRetardant);
