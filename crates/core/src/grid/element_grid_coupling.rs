    ...
    for dz in -cells_radius..=cells_radius {
        for dy in -cells_radius..=cells_radius {
            for dx in -cells_radius..=cells_radius {
                ...
                if ix >= 0 && ix < grid.nx as i32 &&
                   iy >= 0 && iy < grid.ny as i32 &&
                   iz >= 0 && iz < grid.nz as i32 {
...
pub fn transfer_combustion_products_to_grid(
    ...
    if let Some(cell) = grid.cell_at_position_mut(element_pos) {
        ...
        let products = calculate_combustion_products(
            fuel_consumed,
            cell,
...
pub fn apply_grid_to_element(
    element: &mut FuelElement,
    grid: &SimulationGrid,
) {
    ...
    if interpolated.humidity > element.moisture_fraction {
        ...
        let moisture_increase = (interpolated.humidity - element.moisture_fraction) * moisture_uptake_rate;
...
pub fn calculate_buoyancy_on_element(
    element: &FuelElement,
    grid: &SimulationGrid,
) -> Vec3 {
                    ...
                    if target_z >= 0 && target_z < grid.nz as i32 {
                        ...
                        for dy in -spread_radius..=spread_radius {
                            for dx in -spread_radius..=spread_radius {
                                ...
                                if tx >= 0 && tx < grid.nx as i32 &&
                                   ty >= 0 && ty < grid.ny as i32 {
...
mod tests {
    ...
    fn test_combustion_products_transfer() {
        ...
        let co2 = grid.cell_at_position(element.position).unwrap().carbon_dioxide;
