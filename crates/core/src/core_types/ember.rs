...
impl Ember {
    /// Create a new ember
    pub fn new(id: u32, position: Vec3, velocity: Vec3, temperature: f32, mass: f32, source_fuel_type: u8) -> Self {
        Ember {
            id,
            position,
            velocity,
    ...
    pub fn update_physics(&mut self, wind: Vec3, ambient_temp: f32, dt: f32) {
        ...
        let drag_force = 0.5 * AIR_DENSITY * drag_coeff 
                        * relative_velocity.magnitude_squared() * cross_section;
...
pub fn ember_ignition_probability(
    ember: &Ember,
    fuel_receptivity: f32,
) -> f32 {
