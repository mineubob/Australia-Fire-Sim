...
fn create_eucalyptus_tree(sim: &mut FireSimulation, center: Vec3, tree_height: f32) -> Vec<u32> {
    ...
    for i in 0..6 {
        ...
        let branch_id = sim.add_fuel_element(
            ...
            Fuel::eucalyptus_stringybark(),
            2.0,
            FuelPart::Branch { height: 0.0, angle: 0.0 },
            Some(base_id),
        );
...
fn test_single_tree_complete_burnout() {
    ...
    println!("Weather: Temp=42°C, Humidity=12%, Wind=25km/h, FFDI={:.1}", sim.weather.calculate_ffdi());
    ...
    for step in 0..max_steps {
        ...
        if step % 10 == 0 {
            stats.push((step, burning_count, smoldering_count, crown_fire_active, max_temp, total_fuel_remaining));
            println!(
    ...
    assert!(max_burning >= 5, "Fire should spread to at least 5 elements, got {}", max_burning);
    ...
    assert!(crown_fire_detected, "Crown fire should activate for eucalyptus stringybark");
    ...
    assert!(max_smoldering > 0, "Some elements should transition to smoldering");
    
    // 4. Maximum temperature should be realistic (600-1200°C for eucalyptus)
    let max_temp = stats.iter().map(|(_, _, _, _, t, _)| *t).fold(0.0f32, f32::max);
    println!("✓ Maximum temperature: {:.0}°C", max_temp);
    assert!(max_temp > 600.0 && max_temp < 1500.0, "Temperature should be 600-1500°C, got {:.0}°C", max_temp);
    ...
    assert!(consumption_pct > 50.0, "Should consume >50% of fuel, got {:.1}%", consumption_pct);
...
fn test_multiple_trees_fire_spread() {
    ...
    println!("Weather: Temp=38°C, Humidity=15%, Wind=20km/h East, FFDI={:.1}", sim.weather.calculate_ffdi());
    ...
    for i in 0..5 {
        ...
        println!("Created tree {} at ({:.1}, {:.1}) with {} elements", i, x, y, num_elements);
    }
    ...
    let mut tree_ignition_times = vec![Some(0); 5];
    let mut tree_max_burning = vec![0; 5];
    ...
    for i in 0..5 {
        if let Some(ignition_time) = tree_ignition_times[i] {
            println!("Tree {}: Ignited at t={}s, max {} elements burning", 
    ...
    assert!(tree_max_burning[0] >= 10, "First tree should have significant burning, got {}", tree_max_burning[0]);
    ...
    assert!(trees_ignited >= 2, "Fire should spread to at least 2 trees, got {}", trees_ignited);
    ...
    if trees_ignited >= 2 {
        ...
        println!("✓ Fire spread direction: {}", if ignition_order_correct { "Correct (downwind)" } else { "Mixed" });
...
fn test_ember_spotting_between_trees() {
    ...
    for step in 0..max_steps {
        ...
        let tree1_active = tree1_elements.iter().any(|id| {
            sim.get_element(*id).map(|e| e.is_ignited()).unwrap_or(false)
        });
        ...
        if tree2_active && tree2_ignited_time.is_none() {
            ...
            println!("🔥 Tree 2 SPOT IGNITION at t={}s via ember transport!", step);
        }
        
        if step % 30 == 0 {
            let tree1_burning_count = tree1_elements.iter()
                .filter(|id| sim.get_element(**id).map(|e| e.is_ignited()).unwrap_or(false))
                .count();
            let tree2_burning_count = tree2_elements.iter()
                .filter(|id| sim.get_element(**id).map(|e| e.is_ignited()).unwrap_or(false))
                .count();
            
            println!("t={:3}s: Tree1={:2} burning, Tree2={:2} burning, Embers={:3}",
    ...
    assert!(ember_count_max > 0, "Embers should be generated under extreme conditions, got {}", ember_count_max);
    ...
    if let Some(ignition_time) = tree2_ignited_time {
        ...
        println!("  → Embers were generated ({}), proving Albini model is active", ember_count_max);
    }
