...
fn main() {
    ...
    let _id1 = sim.add_fuel_element(Vec3::new(20.0, 25.0, 1.0), fuel.clone(), 3.0, FuelPart::GroundVegetation, None);
    let _id2 = sim.add_fuel_element(Vec3::new(25.0, 25.0, 1.0), fuel.clone(), 3.0, FuelPart::GroundVegetation, None);
    let _id3 = sim.add_fuel_element(Vec3::new(30.0, 25.0, 1.0), fuel.clone(), 3.0, FuelPart::GroundVegetation, None);
    ...
    for step in 0..30 {
        ...
        let grid_temp1 = sim.get_cell_at_position(Vec3::new(20.0, 25.0, 1.0))
            .map(|c| c.temperature())
            .unwrap_or(0.0);
        let grid_temp2 = sim.get_cell_at_position(Vec3::new(25.0, 25.0, 1.0))
            .map(|c| c.temperature())
            .unwrap_or(0.0);
        let grid_temp3 = sim.get_cell_at_position(Vec3::new(30.0, 25.0, 1.0))
            .map(|c| c.temperature())
            .unwrap_or(0.0);
        
        if step % 2 == 0 {
            let stats = sim.get_stats();
            println!("{:4}s | {:7} | {:12} | {:7.0}° | {:7.0}° | {:7.0}°",
                step,
