...
//!
//! Run tests with: cargo test --package fire-sim-core --test australian_bushfire_validation

#[cfg(test)]
mod australian_bushfire_validation {
    use fire_sim_core::{
        physics::{
    ...
    fn test_byram_flame_height_high_extreme() {
        let tests = vec![
            ...
            (100000.0, 18.2, 0.15), // 100,000 kW/m → ~18.2m (Black Saturday levels)
        ];
    ...
    fn test_rothermel_grassland_spread_rate_wind_rule() {
        ...
        let tests = vec![
            ...
            (80.0, 80.0, 270.0), // 80 km/h wind → 80-270 m/min
        ];
    ...
    fn test_van_wagner_critical_surface_intensity() {
        let tests = vec![
            ...
            (0.20, 21000.0, 90.0, 3.0, 20000.0, 35000.0), // Stringybark-like
        ];
    ...
    fn test_albini_lofting_height() {
        let tests = vec![
            (1000.0, 77.0, 0.2),   // 1,000 kW/m → ~77m ±20%
            (5000.0, 185.0, 0.2),  // 5,000 kW/m → ~185m
            (10000.0, 243.0, 0.2), // 10,000 kW/m → ~243m
            (50000.0, 582.0, 0.2), // 50,000 kW/m → ~582m
        ];
    ...
    fn test_albini_spotting_moderate_conditions() {
        ...
        assert!(
            dist >= 500.0 && dist <= 3000.0,
            "Moderate conditions: spotting distance {:.0}m not in expected range 500-3000m",
            dist
        );
    }
    ...
    fn test_albini_spotting_high_intensity() {
        ...
        let dist = calculate_maximum_spotting_distance(intensity, wind, mass, diameter, slope);
        assert!(
            dist >= 2000.0 && dist <= 8000.0,
            "High intensity: spotting distance {:.0}m not in expected range 2000-8000m",
            dist
        );
    }
    ...
    fn test_albini_spotting_extreme_black_saturday() {
        ...
        let dist = calculate_maximum_spotting_distance(intensity, wind, mass, diameter, slope);
        assert!(
            dist >= 5000.0 && dist <= 40000.0,
            "Extreme conditions: spotting distance {:.0}m not in expected range 5-40km (CSIRO: 37km max)",
            dist
        );
    }
    ...
    fn test_stringybark_ladder_fuel_properties() {
        ...
        assert_eq!(
            stringybark.bark_properties.ladder_fuel_factor, 1.0,
    ...
    fn test_regional_weather_temperature_ranges() {
        ...
        assert!(
            summer_min >= 17.0 && summer_min <= 19.0,
            "Perth summer min: expected 18°C, got {:.0}°C",
            summer_min
        );
        assert!(
            summer_max >= 30.0 && summer_max <= 32.0,
            "Perth summer max: expected 31°C, got {:.0}°C",
            summer_max
        );

        // Goldfields - Very hot, arid
        let goldfields = WeatherPreset::goldfields();
        let (gold_summer_min, gold_summer_max) = goldfields.monthly_temps[0];
        assert!(
    ...
    fn test_full_simulation_moderate_conditions() {
        ...
        assert!(
            burning_count >= 2 && burning_count <= 10,
            "Moderate conditions: expected 2-10 burning elements, got {}",
            burning_count
        );
    }
...
// ═══════════════════════════════════════════════════════════════════════════════
// EXAMPLE PROGRAM (for manual validation runs)
// ═══════════════════════════════════════════════════════════════════════════════

fn main() {
    println!("═══════════════════════════════════════════════════════════════════════════════");
    println!("       AUSTRALIAN BUSHFIRE SIMULATION - SCIENTIFIC VALIDATION SUITE            ");
    println!("═══════════════════════════════════════════════════════════════════════════════");
    println!();
    println!("This is now a test suite. Run with:");
    println!("  cargo test --package fire-sim-core --lib australian_bushfire_validation");
    println!();
    ...
    println!(
        "  cargo test --package fire-sim-core --lib australian_bushfire_validation -- --nocapture"
    );
    println!();
    println!("═══════════════════════════════════════════════════════════════════════════════");
}
